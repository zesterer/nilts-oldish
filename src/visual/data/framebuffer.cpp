	...
	namespace Visual
	{
		namespace Data
		{
			FrameBuffer::FrameBuffer()
			{
				...
				glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this->gl_tex_id, 0);

				//Create a depth buffer
				glGenRenderbuffers(1, &this->gl_depth_id);
				//Bind the depth buffer
				glBindRenderbuffer(GL_RENDERBUFFER, this->gl_depth_id);
				glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, 640 * 1.4, 480 * 1.4);
